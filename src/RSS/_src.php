...
define("FEEDCREATOR_VERSION", "FeedCreator 1.7.2-ppt modified by kapxapot");
...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			*/
			if (is_array($this->items[$i]->category) && count($this->items[$i]->category) > 0) {
				foreach ($this->items[$i]->category as $cat_entry) {
					$feed.= "            <category>".htmlspecialchars($cat_entry)."</category>\n";
				}
			}
